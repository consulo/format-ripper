...
class CMSSignedHelper {
  ...
  public AlgorithmIdentifier fixDigestAlgID(AlgorithmIdentifier algId, DigestAlgorithmIdentifierFinder dgstAlgFinder) {
    org.bouncycastle.asn1.ASN1Encodable params = algId.getParameters();
  ...
  public void setSigningEncryptionAlgorithmMapping(ASN1ObjectIdentifier oid, String algorithmName) {
    addEntries(oid, algorithmName);
  }

  public Store<X509CertificateHolder> getCertificates(ASN1Set certSet) {
    if (certSet != null) {
  ...
  public Store<X509AttributeCertificateHolder> getAttributeCertificates(ASN1Set certSet) {
    if (certSet != null) {
      ArrayList<X509AttributeCertificateHolder> certList = new ArrayList<>();
      Enumeration<?> en = certSet.getObjects();
      while (en.hasMoreElements()) {
        org.bouncycastle.asn1.ASN1Primitive obj = ((ASN1Encodable) en.nextElement()).toASN1Primitive();
        if (obj instanceof ASN1TaggedObject) {
          certList.add(new X509AttributeCertificateHolder(
            AttributeCertificate.getInstance(((ASN1TaggedObject) obj).getBaseObject())));
        }
      }
      return new CollectionStore<>(certList);
    }
  ...
  public Store<X509CRLHolder> getCRLs(ASN1Set crlSet) {
    if (crlSet != null) {
      ArrayList<X509CRLHolder> crlList = new ArrayList<>();
  ...
  public Store<?> getOtherRevocationInfo(ASN1ObjectIdentifier otherRevocationInfoFormat, ASN1Set crlSet) {
    if (crlSet != null) {
      ArrayList<ASN1Encodable> crlList = new ArrayList<>();
      Enumeration<?> en = crlSet.getObjects();
      while (en.hasMoreElements()) {
        org.bouncycastle.asn1.ASN1Primitive obj = ((ASN1Encodable) en.nextElement()).toASN1Primitive();
        if (obj instanceof ASN1TaggedObject) {

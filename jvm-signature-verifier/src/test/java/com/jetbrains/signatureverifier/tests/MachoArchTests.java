...
class MachoArchTests {
  ...
  void MachoArchExtractTest(String machoResourceName, long expHeader1, Long expHeader2) throws Exception {
    ...
    List<Long> expectedMachoItems = new ArrayList<>();
    expectedMachoItems.add(expHeader1);
    if (expHeader2 != null) expectedMachoItems.add(expHeader2);

    Assertions.assertEquals(expectedMachoItems, result);
  }

  static Stream<Arguments> MachoArchExtractTestProvider() {
    return Stream.of(
